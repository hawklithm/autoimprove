  ...
  try {
    ...
    if (hasOnnxDep || hasModelFiles) {
      ...
      const confirmed = await confirmPrompt(
        '是否卸载 ONNX local ML 组件？（移除 onnxruntime-node 依赖和模型文件）[y/N] '
      );
...
// -----------------------------------------------------------
async function cleanClaudeMd(claudeDir: string, removedFiles: string[], failedFiles: string[]): Promise<void> {
...
// -----------------------------------------------------------
function listDirectoryContents(dir: string, prefix: string = ''): string[] {
  const items: string[] = [];
...
// -----------------------------------------------------------
async function checkOnnxDependency(packageJsonPath: string): Promise<boolean> {
...
// -----------------------------------------------------------
async function removeOnnxDependency(packageJsonPath: string): Promise<void> {
  const content = JSON.parse(readFileSync(packageJsonPath, 'utf-8'));
