...
export async function setup(options: SetupOptions) {
  ...
  try {
    ...
    cliLogger.error(`❌ Setup failed: ${error?.message ?? error}`);
    cliLogger.error('');
    process.exit(1);
  }
}
...
async function configureMCPServer(mcpServerPath: string, force?: boolean): Promise<void> {
  // Check if already configured — ignoreError: `claude mcp get` returns non-zero
  // when the server doesn't exist yet (e.g. right after uninstall).
  ...
  if (checkResultLower.includes('user config') && !force) {
    cliLogger.print('✓ MCP Server already configured (user-level)');
    return;
  }

  // Remove existing configurations
  if (checkResultLower.includes('user config') || checkResultLower.includes('local config')) {
