...
test_item "MCP config exists" "[ -f ~/.codex/mcp_settings.json ]"
...
test_item "MCP server file exists" "[ -f ~/.autoimprove/src/mcp-server-ts/dist/index.js ]"
test_item "MCP server directory exists" "[ -d ~/.autoimprove/src/mcp-server-ts/dist ]"
echo ""

# 4. Check skills content
echo -e "${YELLOW}4. Skills Content${NC}"
echo "------------------------"
test_item "autoimprove skill dir" "[ -d ~/.codex/skills/autoimprove ]"
test_item "autoimprove skill file" "[ -f ~/.codex/skills/autoimprove/autoimprove.md ]"
echo ""

# 5. Check MCP configuration
echo -e "${YELLOW}5. MCP Configuration${NC}"
echo "------------------------"
test_item "mcp_settings.json is valid JSON" "jq empty ~/.codex/mcp_settings.json 2>/dev/null"
test_item "MCP server configured" "jq '.mcpServers[\"autoimprove\"]' ~/.codex/mcp_settings.json > /dev/null 2>&1"
echo ""
